...
model = YOLO("licence_plate_best.pt")  # fine-tuned weights
reader = easyocr.Reader(['en'], gpu=True)

# Regex pattern: 2 letters + 2 digits + 3 letters (example: MH12ABC)
...
def recognize_plate(plate_crop):
    ...
    try:
        # FIXED: added missing comma and corrected keyword
...
out = cv2.VideoWriter(
    output_video, fourcc, cap.get(cv2.CAP_PROP_FPS),
...
# ...existing code...
            # Overlay zoomed-in plate in top-left corner (fixed position)
            # if plate_crop.size > 0:
            #     overlay_h, overlay_w = 60, 160  # smaller overlay
            #     plate_resized = cv2.resize(plate_crop, (overlay_w, overlay_h))

            #     # Place overlay at top-left corner
            #     oy1, ox1 = 10, 10
            ...
            #         if stable_text:
            #             cv2.putText(frame, stable_text, (ox1, oy1 - 5),
...
# ...existing code...
    ...
    # Resize the frame to a smaller size before displaying
    resized_frame = cv2.resize(frame, (960, 540))  # You can adjust size
